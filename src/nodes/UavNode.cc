...
// Mobility was removed from this track. Its hand-rolled trajectory generator
// (scheduleMobilityTick/advanceMobility) is gone with it, and so are the
// mobilityModel/mobilitySpeedMps/mobilityHeadingDeg parameters and the
// mobilit  state they drove. Motion is measured on the INET track instead
// (InetMobility*SHA3), where it uses INET's own mobility models and changes
// the actual 802.11 channel rather than only a microsecond-scale propagation
// term. See simulations/omnetpp.ini's mobility note.
...
void UavNode::finish() {
    ...
    // Positions only. The mobilityModel/totalDistanceTraveledM scalars are gone
    // with the mobility code; motion is measured on the INET track.
