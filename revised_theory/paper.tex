...
Two observations follow, both of which bear on how the literature reports these
costs.
...
Tables~\ref{tab:cmp_latency}--\ref{tab:cmp_security} position the protocol against
published PUF-based UAV schemes and against conventional public-key and
pre-shared-key baselines.

Two caveats govern how these tables should be read. First, figures taken from the
literature were obtained on the authors' own platforms --- typically ARM
Cortex-A-class processors or Raspberry Pi boards --- whereas ours are measured on an
x86-64 host. Cross-platform latency comparisons therefore conflate protocol
efficiency with processor class, and the platform column is given so that no such
comparison is made implicitly. The public-key rows in
Table~\ref{tab:cmp_latency} are the exception: those we re-measured ourselves on
our own platform (\S\ref{subsec:baselines}), and only those are directly
comparable with our own rows. Second, communication and storage costs are
platform-independent and can be compared directly.

\begin{table}[H]
\centering
\caption{Authentication latency. Only the rows marked \emph{same platform} were
measured on our host and are directly comparable with each other}
\label{tab:cmp_latency}
\small
\begin{tabular}{@{}p{3.5cm}p{3.0cm}p{2.6cm}c@{}}
\toprule
\textbf{Scheme} & \textbf{Platform} & \textbf{Device$\leftrightarrow$server} &
\textbf{Peer} \\
\midrule
RSA-2048 (sign+verify) & same platform & $1.08$~ms & --- \\
ECDSA P-256 (sign+verify) & same platform & $0.15$~ms & --- \\
ECDH P-256 (derive) & same platform & $0.09$~ms & --- \\
\midrule
Gope~\cite{puf_lightweight_uav} & ARM (reported) & $10$--$50$~ms & --- \\
Li~\cite{puf_uav_cross_domain} & ARM (reported) & $45$--$80$~ms & partial \\
Khan~\cite{puf_6g_uav} & ARM (reported) & $5$--$20$~ms & --- \\
Bera~\cite{peer2peer_auth} & ARM (reported) & $<1$~ms & $<1$~ms \\
\midrule
\textbf{This work} (\texttt{sha3}) & \textbf{same platform} &
$\mathbf{1.50}$~\textbf{ms} & $\mathbf{0.42}$~\textbf{ms} \\
\textbf{This work} (\texttt{spongent}) & \textbf{same platform} &
$\mathbf{1.51}$~\textbf{ms} & $\mathbf{0.42}$~\textbf{ms} \\
\bottomrule
\end{tabular}
\normalsize
\end{table}

Our end-to-end figure includes one PUF evaluation and one fuzzy-extractor
reproduction ($\approx 1.4$~ms of the $1.50$~ms total, Table~\ref{tab:latency}),
which the public-key baselines do not perform at all. Excluding that
device-specific work, the cryptographic handshake costs approximately $0.2$~ms and
is thus of the same order as an elliptic-curve exchange. We do not claim an
order-of-magnitude latency advantage; \S\ref{subsec:baselines} sets out why such
claims are unsound on this evidence.

\begin{table}[H]
\centering
\caption{Communication overhead for full mutual authentication (platform
independent)}
\label{tab:cmp_overhead}
\small
\begin{tabular}{@{}p{3.5cm}ccc@{}}
\toprule
\textbf{Scheme} & \textbf{Msgs} & \textbf{Device$\leftrightarrow$server} &
\textbf{Peer pair} \\
\midrule
RSA-2048 PKI (with cert) & 2--4 & $768$--$1024$~B & $768$~B \\
ECC-256 (with cert) & 2--4 & $384$--$512$~B & $384$~B \\
PSK (AES-GCM) & 2 & $\approx 256$~B & $\approx 256$~B \\
Gope~\cite{puf_lightweight_uav} & 3 & $\approx 200$~B & --- \\
Li~\cite{puf_uav_cross_domain} & 4 & $\approx 300$~B & --- \\
\midrule
\textbf{This work}, $N{=}5$ & 4 & $507$~B & $197$~B \\
\textbf{This work}, $N{=}10$ & 4 & $777$~B & $197$~B \\
\textbf{This work}, $N{=}20$ & 4 & $1317$~B & $197$~B \\
\bottomrule
\end{tabular}
\normalsize
\end{table}

Peer authentication at $197$~B per pair is below every listed alternative, and it
requires no ground-station involvement. The device--server figure grows with swarm
size because $M_4$ carries one credential per peer: at $N=10$ the credential
package is $545$~B of the $777$~B total. Issuing credentials on demand rather than
in bulk removes that growth at the cost of an extra round trip per new peer, and is
the appropriate configuration for large swarms.

\begin{table}[H]
\centering
\caption{Per-device storage for an $N$-UAV swarm. \emph{Secret} counts material
whose disclosure compromises authentication}
\label{tab:cmp_storage}
\small
\begin{tabular}{@{}p{3.4cm}p{4.2cm}cc@{}}
\toprule
\textbf{Scheme} & \textbf{Stored on device} & \textbf{Total} & \textbf{Secret} \\
\midrule
RSA-2048 PKI & private key + cert chain & $\approx 1.5$~KB & $256$~B \\
ECC-256 & private key + cert & $\approx 0.4$~KB & $32$~B \\
PSK & $N-1$ pairwise keys & $32(N{-}1)$~B & all \\
Gope~\cite{puf_lightweight_uav} & helper data + identity & $\approx 0.1$~KB & none \\
\midrule
\textbf{This work} & identity, challenge, helper data
($4\times32$~B sketch $+$ seed) $+$ $N{-}1$ peer credentials &
$\approx 0.2 + 0.03N$~KB & $32(N{-}1)$~B \\
\bottomrule
\end{tabular}
\normalsize
\end{table}

The device holds no long-term secret of its own: identity, challenge and helper
data are public by construction, and the master key exists only transiently, being
regenerated from the PUF on demand. Per-pair credentials are secret, but each is
confined to one link, so the disclosure of a device's storage costs $N-1$ links
rather than the whole mesh --- the property measured in
\S\ref{subsec:security_eval}. A pre-shared-key deployment stores the same quantity
of pairwise material but derives it from no physical root, so a captured device
also yields a usable long-term identity.

\begin{table}[H]
\centering
\caption{Security properties. \emph{Ephemeral} indicates forward secrecy from a
per-session key exchange}
\label{tab:cmp_security}
\small
\begin{tabular}{@{}p{2.9cm}ccccc@{}}
\toprule
\textbf{Scheme} & \textbf{No stored} & \textbf{Mutual} & \textbf{Ephemeral} &
\textbf{GS-free} & \textbf{Capture} \\
 & \textbf{secret} & \textbf{auth} & \textbf{(fwd sec)} & \textbf{peer} &
\textbf{blast radius} \\
\midrule
RSA-2048 PKI & No & Yes & Optional & Yes & 1 device \\
ECC-256 & No & Yes & Optional & Yes & 1 device \\
PSK (AES-GCM) & No & Yes & No & Yes & whole swarm \\
Gope~\cite{puf_lightweight_uav} & Yes & Yes & No & No & 1 device \\
Li~\cite{puf_uav_cross_domain} & Yes & Yes & No & Partial & 1 device \\
Khan~\cite{puf_6g_uav} & Yes & Yes & Yes & No & 1 device \\
Bera~\cite{peer2peer_auth} & No & Yes & No & Yes & whole swarm \\
\midrule
\textbf{This work} & \textbf{Yes} & \textbf{Yes} & \textbf{Mandatory} &
\textbf{Yes} & $\mathbf{N-1}$ \textbf{links} \\
\bottomrule
\end{tabular}
\normalsize
\end{table}

The distinguishing combination is GS-free peer authentication rooted in per-pair
credentials, together with a mandatory rather than optional ephemeral exchange.
Schemes that root peer authentication in a swarm-wide secret obtain low peer cost
but lose all isolation on capture; schemes that route peer authentication through
the ground station retain isolation but cannot operate while that link is
unavailable. Making the ephemeral exchange mandatory rather than optional costs
approximately $190~\mu$s per handshake (Table~\ref{tab:pki}) and is the reason we
do not claim a latency advantage over elliptic-curve schemes.
...
ground-station messages. The final row is the P3 ablation, in which the
verify-before-respond rule is disabled to validate the adversary}
\label{tab:attacks}
\small
...
\midrule
GS impersonation & full protocol & 8 & 0 & 0 & fail & yes \\
$M_1$ replay & full protocol & 1 & 0 & 0 & fail & yes \\
Credential sniffing & full protocol & 10 & 0 & 0 & fail & yes \\
\midrule
GS impersonation & \emph{ablation: no P3} & 8 & 1 & 1 & \emph{succeed} & yes \\
\bottomrule
\end{tabular}
\end{table}

Every outcome matches its declared expectation. Under the full protocol, eight
forged ground-station messages produced no response of any kind: the UAV verifies
the authenticator before performing any PUF-dependent computation, so no
...
challenge--response pair, which establishes two things: the adversary detects a
success when one exists, so the zeros above are evidence rather than an artefact;
and the verify-before-respond rule --- not some incidental property of the
implementation --- is what closes the oracle.
