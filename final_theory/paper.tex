...
twenty-drone swarm across thirty repetitions, at $1.50 \pm 0.002$~ms per
UAV--GS handshake and $0.417 \pm 0.001$~ms per peer pair, with every primitive
...
figure is used when accounting for the measured end-to-end latency of
...
work (mean $\pm$ 95\% CI, thirty runs each). Compute and Network are reported
on the same convention as Table~\ref{tab:latency}}
\label{tab:baseline_comparison}
\small
\begin{tabular}{@{}p{3.6cm}cccc@{}}
\toprule
\textbf{Protocol} & \textbf{End-to-end (ms)} & \textbf{Compute (ms)} &
\textbf{Network (ms)} & \textbf{Overhead (B)} \\
\midrule
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Configuration} & \textbf{End-to-end (ms)} & \textbf{Success} & \textbf{vs.\ idealised medium} \\
\midrule
...
twenty-drone swarm authenticate, at $1.50 \pm 0.002$~ms per UAV--GS handshake and
$0.417 \pm 0.001$~ms per peer pair over thirty independent repetitions. Every
