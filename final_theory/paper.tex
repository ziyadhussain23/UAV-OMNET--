...
twenty-drone swarm across thirty repetitions, at a simulated elapsed time of
$1.50 \pm 0.002$~ms per UAV--GS handshake and $0.417 \pm 0.001$~ms per peer
pair, with every primitive checked against published reference values. Executed attacks confirm the analysis: forged ground-station
...
falsifiable. Measured on the same host as RSA-2048 and elliptic-curve
baselines, the ephemeral key exchange this protocol mandates for forward
secrecy costs about as much as a bare ECDH/ECDSA exchange, not an order of
magnitude less. A full same-platform handshake-to-handshake comparison against
real RSA-authenticated and ECDSA-authenticated ephemeral-DH protocols places
this work at $2.68$~ms against $2.05$~ms (RSA) and $0.83$~ms (ECDSA): closer to
RSA than a primitive-level reading alone would suggest, and genuinely slower
than ECDSA, the disclosed cost of storing no long-term secret on the device.
This protocol's advantages are properties rather than raw
speed: no stored long-term secret, no certificate infrastructure, GS-free peer
authentication, and capture damage bounded to a node's own links.
\end{abstract}
...
rather than to a confound. Table~\ref{tab:scenarios} lists all seventeen
...
\label{tab:scenarios}
\footnotesize
\begin{tabular}{@{}p{2.7cm}p{4.6cm}p{5.4cm}p{1.6cm}@{}}
\toprule
...
\texttt{MobilityLinear} & UAV motion: constant velocity, boundary bounce & Does physical movement disturb the protocol's timing or correctness? & Tab.~\ref{tab:mobility} \\
\texttt{MobilityRandomWalk} & UAV motion: random heading redrawn every $100$~ms & Cross-check of the mobility finding under a second, statistically different motion pattern & Tab.~\ref{tab:mobility} \\
\texttt{Inet80211SHA3} & network layer: idealised delay $\rightarrow$ real IEEE~802.11 CSMA/CA, \texttt{sha3} suite & What does genuine MAC-layer contention add to the idealised lower bound? & Tab.~\ref{tab:inet} \\
\texttt{Inet80211SPONGENT} & network layer as above, \texttt{spongent} suite & Confirms the contention cost is suite-independent & Tab.~\ref{tab:inet} \\
...
\texttt{AtkLegacyGsImpersonate} & same attacker, verify-before-respond (P3) disabled & Negative control: does the same attacker succeed once the one relevant defence is removed? & Tab.~\ref{tab:attacks} \\
...
\texttt{AtkCredentialSniff} & adversary present: passive read of $M_4$'s credential package & Does authenticated encryption keep the per-pair credential confidential in transit? & Tab.~\ref{tab:attacks} \\
\bottomrule
\end{tabular}
\end{table}

Grouped by purpose, the seventeen scenarios fall into five families: a
...
\emph{deployment-realism pair} (mobility and INET) that each bracket one
simplification of the idealised model, and a \emph{security family} of four
attack configurations built around one negative control. No scenario duplicates
another's variable: the two mobility models and the two INET suites are kept
deliberately minimal precisely because \S\ref{subsec:mobility} and
\S\ref{subsec:inet} show their outcomes agree with each other to within
confidence interval, which is itself part of the finding, not an omission.
...
Table~\ref{tab:latency} reports the Phase-2 handshake's simulated elapsed time,
together with its compute and network components. These three columns are
\emph{not} one figure split into two: Compute and Network are independent
measurements on two different clocks (host wall-clock timers for Compute,
the simulator's own clock for Network), and adding them gives the total a real
device would pay if it ran the two costs back to back --- a figure this paper
uses explicitly in \S\ref{subsec:baseline_comparison}, never silently. Simulated
elapsed is the third, separate quantity: the OMNeT++ simulation clock's own
start-to-finish reading for the handshake, which does not advance while host
compute time is being measured, so it is close to the Network column and
systematically smaller than Compute$+$Network.

\begin{table}[H]
\centering
\caption{Phase-2 latency and overhead (mean $\pm$ 95\% CI, thirty runs).
\emph{Simulated elapsed} is the simulator-clock reading for the whole
handshake, not Compute$+$Network --- see \S\ref{subsec:latency} for why the
three columns do not sum}
\label{tab:latency}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Configuration} & \textbf{Simulated elapsed (ms)} & \textbf{Compute (ms)} &
\textbf{Network (ms)} & \textbf{Overhead (B)} \\
\midrule
StadiumSHA3     & $1.5028 \pm 0.0023$ & $1.6024 \pm 0.0322$ & $1.0737 \pm 0.0020$ & $777$ \\
StadiumSPONGENT & $1.5082 \pm 0.0023$ & $4.4533 \pm 0.0306$ & $1.0790 \pm 0.0020$ & $781$ \\
Baseline5UAV    & $1.1442 \pm 0.0039$ & $1.5908 \pm 0.0245$ & $0.7142 \pm 0.0027$ & $507$ \\
Swarm20         & $2.2221 \pm 0.0023$ & $1.5757 \pm 0.0170$ & $1.7929 \pm 0.0015$ & $1317$ \\
\bottomrule
\end{tabular}
\end{table}

Phase-3 peer authentication costs $0.4167 \pm 0.0006$~ms per pair, pooled across
...
($0.4171$, $0.4171$, $0.4163$, and $0.4164$~ms respectively) agree to within their
...
Table~\ref{tab:latency} reports Phase~2 as three summary numbers. This still
...
\midrule
$M_1$ & UAV: PUF eval.\ + \texttt{Rep} + MAC $\sigma_1$ (sender compute) & $1.4330$ & $1.9891$ & $0.2554$ & $104$ \\
      & \quad of which: PUF evaluation & $1.1532$ & $1.1317$ & --- & --- \\
      & \quad of which: BCH decode & $0.2068$ & $0.4915$ & --- & --- \\
      & GS: MAC $\sigma_1$ verify (receiver compute) & $0.0503$ & $0.5859$ & --- & --- \\
$M_2$ & UAV: MAC $\sigma_2$ verify (verify-before-respond gate) & $0.0799$ & $1.1683$ & $0.2303$ & $85$ \\
$M_3$ & UAV: MAC $\sigma_3$ compute (sender compute) & $0.0799$ & $1.1683$ & $0.1737$ & $43$ \\
      & GS: MAC $\sigma_3$ verify $+$ session-key KDF (receiver compute) & $0.1352$ & $2.3811$ & --- & --- \\
$M_4$ & GS: AEAD-seal credential package (sender compute) & $0.0102$ & $0.1277$ & $0.8433$ & $545$/$549$ \\
\midrule
\textbf{Total} & & $\mathbf{1.6024}$ & $\mathbf{4.4533}$ & $\mathbf{1.0737}$/$\mathbf{1.0790}$ & $\mathbf{777}$/$\mathbf{781}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{$M_1$ is disproportionately expensive for a structural reason, not a
performance one.} Table~\ref{tab:msgbreakdown}'s own rows show that $69$--$71\%$
of $M_1$'s compute time under either suite is the PUF evaluation and BCH
reproduction (\emph{deviation \#1} of the specification: the paper's phase
ordering implies \texttt{Rep} runs at $M_3$, but the UAV needs $\text{mk}_i$ to
authenticate $M_1$ itself, so \texttt{Rep} necessarily runs before $M_1$ is
sent). This is why $M_1$, not $M_3$ where the paper's narrative places the key
regeneration, carries the PUF cost in the measured breakdown.
...
of which the suite changes materially. Every millisecond of the $1.6$~ms
versus $4.5$~ms compute gap is therefore attributable to the cryptographic suite
alone, not to any difference in the network path.
...
costs roughly $14$--$15\times$ \texttt{sha3}'s HMAC on these two messages, consistent
with the $228~\mu$s-versus-$2.2~\mu$s per-call gap in
Table~\ref{tab:primitives}. $M_1$'s gap is compressed by the shared PUF cost sitting
...
$P_2$ & UAV$_j$: MAC $\tau_1$ verify $+$ MAC $\tau_2$ compute (responder) & $0.0239$ & $0.2791$ & $0.1107$ \\
$P_3$ & UAV$_i$: MAC $\tau_2$ verify $+$ MAC $\tau_3$ compute $+$ session KDF (initiator) & $0.0402$ & $0.5241$ & $0.0743$ \\
\midrule
...
$\text{Cred}_{ij}$ --- which is why Phase-3 latency ($0.417$~ms, \texttt{sha3}) is
roughly a third of Phase-2's ($1.503$~ms): Phase~3 never re-pays the PUF cost
that dominates $M_1$. Second, the \texttt{spongent} suite's Phase-3 total
($1.372$~ms) is $3.3\times$ \texttt{sha3}'s, a larger relative gap than Phase-2's
$2.96\times$ ($5.4$ against $1.6$~ms of compute), because Phase~3 has no PUF or
AEAD cost to dilute the three consecutive MAC calls that dominate the exchange.
...
dedicated tight-loop benchmark against the same library. The two were kept in
separate tables in an earlier draft, which made the comparison a reader
actually wants --- our symmetric primitives against their asymmetric ones ---
harder to see than it should be. They are merged here for that reason.
...
AEAD seal & & 2.52 & 1.63 \\
AEAD open & & 1.63 & 1.45 \\
X25519 keygen$^{\dagger}$ & & 34.00 & 33.97 \\
X25519 derive$^{\dagger}$ & & 64.21 & 64.56 \\
\midrule
...
X25519 & keygen$^{\dagger}$ & 65.6 & 66.3 \\
X25519 & derive$^{\dagger}$ & 125.7 & 127.9 \\
\bottomrule
\end{tabular}

\vspace{2pt}
\raggedright\footnotesize
$^{\dagger}$X25519 is measured twice, deliberately, because it is the one
operation both blocks share. Both measurements call the identical
\texttt{x25519Generate}/\texttt{x25519Derive} code; the roughly two-fold gap
between them ($34$--$65~\mu$s in-protocol versus $66$--$126~\mu$s benchmarked)
is a measurement-context effect --- ten calls interleaved with the rest of a
simulated handshake versus a 2000-iteration tight loop --- not two different
implementations. The benchmarked figure is used wherever this work's X25519
cost is compared against the ECDH/ECDSA rows below, since only figures taken
under identical tight-loop conditions are directly comparable; the in-protocol
figure is used when accounting for the measured Compute column of
...
\addplot[fill=blue!55] coordinates
  {(Hash,1.00) (MAC,2.20) (KDF,4.93) (Extract,4.04) (AEAD seal,2.52) (AEAD open,1.63)
   (X25519 keygen,34.00) (X25519 derive,64.21)};
\addplot[fill=orange!75] coordinates
  {(MAC,228.27) (KDF,139.32) (Extract,292.25) (AEAD seal,1.63) (AEAD open,1.45)
   (X25519 keygen,33.97) (X25519 derive,64.56)};
\addplot[fill=teal!60] coordinates
  {(RSA sign,1048.6) (RSA verify,32.0) (ECDSA sign,43.6) (ECDSA verify,109.0)
   (ECDH derive,91.0) (X25519 keygen,65.6) (X25519 derive,125.7)};
...
primitive (KDF, $4.9~\mu$s) is still an order of magnitude below the cheapest
baseline operation shown (RSA-2048 verify, $32~\mu$s). This is where our own
values are genuinely, unambiguously better, and it is a fair comparison
because both sides are single, isolated primitive calls measured the same way.

\textbf{The \texttt{spongent}-suite does not carry the same advantage.} Its
MAC, KDF, and Extract operations ($139$--$292~\mu$s) land inside the same
range as ECDSA sign/verify and ECDH derive ($44$--$109~\mu$s) rather than below
them --- the direct, disclosed cost of a hardware-area-optimised permutation
running in software. Only \texttt{spongent}'s AEAD operations ($1.5$--$1.6~\mu$s,
via Ascon) stay cheap regardless of suite.
...
work (mean $\pm$ 95\% CI, thirty runs each). Simulated elapsed, Compute, and
Network are reported on the same convention as Table~\ref{tab:latency}; the
realistic total used in the discussion below is Compute$+$Network, not the
Simulated elapsed column}
\label{tab:baseline_comparison}
\small
\begin{tabular}{@{}p{3.6cm}cccc@{}}
\toprule
\textbf{Protocol} & \textbf{Simulated elapsed (ms)} & \textbf{Compute (ms)} &
\textbf{Network (ms)} & \textbf{Overhead (B)} \\
\midrule
RSA-2048-signed ephemeral DH & $1.4042 \pm 0.0023$ & $1.3558 \pm 0.2129$ & $0.6990 \pm 0.0020$ & $703$ \\
ECDSA-P256-signed ephemeral DH & $0.9109 \pm 0.0023$ & $0.3805 \pm 0.0089$ & $0.4523 \pm 0.0020$ & $330$--$335$ \\
\midrule
\textbf{This work} (\texttt{sha3}) & $1.5028 \pm 0.0023$ & $1.6024 \pm 0.0322$ & $1.0737 \pm 0.0020$ & $777$ \\
\textbf{This work} (\texttt{spongent}) & $1.5082 \pm 0.0023$ & $4.4533 \pm 0.0306$ & $1.0790 \pm 0.0020$ & $781$ \\
\bottomrule
\end{tabular}
\end{table}

Reading Compute$+$Network as one realistic total, the same convention
\S\ref{subsec:latency} already uses: the RSA baseline totals $2.05$~ms, the
ECDSA baseline $0.83$~ms, this work's \texttt{sha3} profile $2.68$~ms, and its
\texttt{spongent} profile $5.53$~ms.

Two findings follow, and they point in different directions --- reported as
measured, not smoothed into a single verdict.

\textbf{Against ECDSA, this work is genuinely, substantially slower.} $0.83$~ms
against $2.68$~ms is a real, disclosed gap of about $3.2\times$. ECDSA-P256
signing is cheap ($44~\mu$s, Table~\ref{tab:primitives}), so a real
ECDSA-authenticated handshake pays almost nothing beyond the ephemeral DH
exchange itself; this work additionally pays the PUF-evaluation-and-fuzzy-
extractor-reproduction cost every single handshake, in exchange for storing no
long-term secret on the device at all. That is the honest price of this
work's threat model against a scheme whose signing key is cheap to use but
must be stored somewhere a captured device can expose it.

\textbf{Against RSA, this work is close to comparable, not order-of-magnitude
slower.} $2.05$~ms against $2.68$~ms is a real, but far narrower, gap than the
ECDSA comparison, and far narrower than a primitive-level reading alone would
suggest. The reason is directly visible in Table~\ref{tab:baseline_comparison}'s
own Compute column: RSA-2048 signing costs $1.03$~ms per handshake (measured
in-protocol, consistent with Table~\ref{tab:pki}'s $1048.6~\mu$s tight-loop
figure), which is close to this work's own PUF-evaluation-plus-fuzzy-extractor
cost ($\approx 1.4$~ms, Table~\ref{tab:latency}'s underlying components). A
full RSA-authenticated handshake is not a cheap baseline to compare against
once RSA's own signing cost is actually charged to it, rather than compared
primitive-for-primitive against a single MAC call.
...
signature ($330$--$335$~B, DER-encoded $r,s$ components of slightly differing
length run to run) is the cheapest of the four rows; this work's $777$/$781$-byte
total includes a per-peer credential package that neither baseline carries,
since GS-free peer authentication is a property neither baseline has at all
...
Every configuration so far holds each UAV at a fixed perimeter coordinate. Two
further configurations move them instead, over thirty repetitions each:
\texttt{MobilityLinear} (constant velocity, reflecting off the field boundary)
and \texttt{MobilityRandomWalk} (a new random heading drawn every $100$~ms).
Both use the same link-delay model as \S\ref{subsec:simmodel}, which reads each
UAV's position afresh for every message, so a moving UAV's propagation delay is
recomputed, not cached, at the moment each message is sent.

\begin{table}[H]
\centering
\caption{UAV mobility: distance travelled and Phase-2 latency, thirty runs each}
\label{tab:mobility}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Mobility model} & \textbf{Speed} & \textbf{Mean distance travelled} &
\textbf{Phase-2 simulated elapsed (ms)} \\
\midrule
Linear      & $8$~m/s & $2398.7$~m (of $2400$~m theoretical) & $1.5026 \pm 0.0023$ \\
Random walk & $8$~m/s & $2396.2$~m (of $2400$~m theoretical) & $1.5028 \pm 0.0023$ \\
\bottomrule
\end{tabular}
\end{table}

The measured distance matches the theoretical $8~\text{m/s} \times 300~\text{s} =
2400$~m travel budget closely for both models, confirming the mobility update
loop is actually moving each UAV rather than merely being configured to. Phase-2
latency, however, is statistically indistinguishable from the static baseline
($1.5028 \pm 0.0023$~ms, Table~\ref{tab:latency}) for both mobility models. This
is expected, not a null result to hide: a Phase-2 handshake completes in
$\approx 1.5$~ms of simulated time, during which a UAV travelling at $8$~m/s
covers about $1.2$~cm --- negligible against the $500\times300$~m field, so
propagation delay does not change measurably within the span of one handshake.
Mobility's effect on latency, if any, would appear across many independent
re-authentications separated in time as a UAV patrols far from the ground
station, not within a single handshake; that longitudinal question is left for
future work.
...
bracket that bound with a measured upper one, the same Phase-2 handshake ---
identical protocol code, identical crypto suites, identical PUF models --- was
also driven over a real IEEE~802.11 ad-hoc stack (the INET framework's
\texttt{Ieee80211ScalarRadioMedium}, with genuine CSMA/CA contention, backoff,
and retransmission) instead of the idealised medium. This track is deliberately
scoped to Phase-2 latency and throughput only: the passive/active adversary
...
a real gate/channel topology where hosts communicate directly, so no attack
results are claimed on this track. UAV positions are held static (the same
perimeter layout as \texttt{StadiumSHA3}) so the comparison isolates the
MAC-layer effect rather than conflating it with mobility.

\begin{table}[H]
\centering
\caption{Phase-2 latency over a real 802.11 stack versus the idealised medium,
thirty runs each}
\label{tab:inet}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Configuration} & \textbf{Simulated elapsed (ms)} & \textbf{Success} & \textbf{vs.\ idealised medium} \\
\midrule
Inet80211SHA3     & $11.722 \pm 0.408$ & $298/300$ & $7.8\times$ \\
Inet80211SPONGENT & $11.744 \pm 0.409$ & $298/300$ & $7.8\times$ \\
\midrule
StadiumSHA3 (idealised, Table~\ref{tab:latency}) & $1.503 \pm 0.002$ & $300/300$ & --- \\
\bottomrule
\end{tabular}
\end{table}

Real CSMA/CA contention costs approximately $7.8\times$ the idealised medium's
latency for the identical handshake --- a genuine, disclosed price of MAC-layer
realism that the primary model, by construction, cannot show. The two crypto
suites remain within their own confidence intervals of each other on this track
too, consistent with \S\ref{subsec:baselines}'s finding that suite choice barely
affects network-bound latency.

The two non-participating UAVs (of $300$ across both configurations, one per
configuration) are not a MAC-layer artefact: both show zero bytes sent and a
ground-station attempt count one short of the full ten, meaning the UAV's own
fuzzy-extractor reproduction failed before it ever transmitted $M_1$ --- the
same class of event already reported for the \texttt{ArbiterPuf} and
\texttt{HighNoise} configurations (\S\ref{subsec:functional}), not a new failure
mode introduced by the 802.11 stack. Both UAVs use the same idealised PUF model
and the same $3\%$ bit-error rate as \texttt{StadiumSHA3}, which observed zero
such failures in $300$ devices; two failures in a further $300$ independent
draws of the same low-probability event is consistent with ordinary sampling
variance, not a rate change.
...
membership (join and leave during flight) is left for future work; mobility
itself is now measured (\S\ref{subsec:mobility}). The full-protocol latency
comparison against RSA- and ECDSA-authenticated ephemeral key exchange is now
measured as well (\S\ref{subsec:baseline_comparison}), rather than left at the
same-platform \emph{primitive}-level costs alone (\S\ref{subsec:baselines}).
...
$1.50 \pm 0.002$~ms per UAV--GS handshake and $0.417 \pm 0.001$~ms per peer
pair over thirty independent repetitions. Every
...
$3.2\times$ slower ($2.68$ against $0.83$~ms), the disclosed cost of storing no
long-term secret on the device against a scheme whose signing key must be
stored somewhere; against RSA the gap narrows to $2.68$ against $2.05$~ms,
because RSA-2048's own signing cost ($\approx 1.03$~ms per handshake) is close
to this protocol's PUF-evaluation-and-fuzzy-extractor cost, so a fully-costed
RSA baseline is not the cheap comparison a primitive-level reading alone would
suggest. This protocol's advantages are consequently argued as properties
...
capture damage bounded to a node's own links, and a symmetric core small
